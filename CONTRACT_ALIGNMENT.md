...
## Response and KYC types

`KycVerification.kyc_proof` accepts provider and reference ID evidence; it is required by the server for `THIRD_PARTY`. The reference length is 10–64 characters. The SDK does not truncate it or infer environment rollout.

Response types allow null payment metadata, next action, latest attempt and card risk controls; empty advice/CVV/gender values; and an absent product mode. Deposit types expose deposit method and sender classifications. Request types retain their existing constraints.
...
`CheckBeneficiaryParams` is now a union requiring an account number or IBAN; consumers extending the old interface should use a type intersection.
